...
public class MultiOption extends ClickGuiOption {
    ...
    protected void renderControl(GuiGraphics graphics, float x, float y, float width, float height, float scale, float alpha) {
        ...
        String counter = setting.selectedCount() + " из " + values.length;
        float counterWidth = Render2D.textWidth(FontType.SEMIBOLD, counter, titleSize);
        float titleWidth = Math.max(1.0f, width - counterWidth - 4.0f * scale);
        renderScrollingText(graphics, "multi:title", displayName(), x, y, titleSize,
                color(236, 237, 247, 250, alpha),
                x, y, titleWidth, CHIPS_TOP * scale,
                x, y, titleWidth, CHIPS_TOP * scale);
        Render2D.text(FontType.SEMIBOLD, counter, x + width - counterWidth, y, titleSize,
                color(139, 139, 162, 220, alpha));
