...
final class BlurRenderState implements GuiElementRenderState {
    private final BuiltBlur blur;
    private final Matrix3x2f pose;
    private final ScreenRectangle scissorArea;
    private final ScreenRectangle bounds;

    BlurRenderState(Matrix3x2f pose, BuiltBlur blur, ScreenRectangle scissorArea) {
        this.blur = blur;
        this.pose = pose;
    ...
    private void vertex(VertexConsumer consumer, float x, float y, int color, int batchIndex) {
        consumer.addVertexWith2DPose(pose, x, y).setColor(color).setLineWidth((float) (batchIndex + 1));
    }
    ...
    public TextureSetup textureSetup() {
        return BlurFramebuffer.getInstance().textureSetup(blur.algorithm());
    }
