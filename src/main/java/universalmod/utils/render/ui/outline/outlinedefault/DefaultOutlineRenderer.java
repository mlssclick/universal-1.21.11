...
public final class DefaultOutlineRenderer implements AutoCloseable {
    ...
    public static final RenderPipeline OUTLINE_PIPELINE = RenderPipeline.builder()
            .withLocation(id("pipeline/outline_default"))
            .withVertexShader(id("ui/shared/outline_quad"))
            .withFragmentShader(id("ui/outline_default/outline_default"))
            ...
            .withUniform("OutlineParamsArray", com.mojang.blaze3d.shaders.UniformType.UNIFORM_BUFFER)
            .build();
    ...
    public void draw(GuiGraphics graphics, BuiltOutline outline) {
        beginFrame(graphics);
        enqueue(outline);
        flush();
    }
    ...
    private void submit(GuiGraphics graphics, BuiltOutline outline) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor) graphics)
                    ...
                    .submitGuiElement(new DefaultOutlineRenderState(pose, normalized, ScissorUtil.current()));
        } catch (RuntimeException ignored) {
        }
    }
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer uniformData = buildUniformData(stack, preparedOutlines);
            paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "UNIVERSALMOD_outline_default_params", GpuBuffer.USAGE_UNIFORM, uniformData);
            paramsDirty = false;
            return paramsBuffer;
        }
    }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        try {
            paramsBuffer = RenderSystem.getDevice().createBuffer(
                    () -> "UNIVERSALMOD_outline_default_params",
