...
public final class GlassOutlineRenderer implements AutoCloseable {
    ...
    public static final RenderPipeline GLASS_OUTLINE_PIPELINE = RenderPipeline.builder()
            .withLocation(id("pipeline/outline_glass"))
            .withVertexShader(id("ui/outline_glass/outline_glass"))
            .withFragmentShader(id("ui/outline_glass/outline_glass"))
            ...
            .withUniform("GlassOutlineParamsArray", com.mojang.blaze3d.shaders.UniformType.UNIFORM_BUFFER)
    ...
    public void draw(GuiGraphics graphics, BuiltGlassOutline outline) {
        beginFrame(graphics);
        enqueue(outline);
        flush();
    }
    ...
    private void submit(GuiGraphics graphics, BuiltGlassOutline outline) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor) graphics)
                    ...
                    .submitGuiElement(new GlassOutlineRenderState(pose, normalized, ScissorUtil.current()));
        } catch (RuntimeException ignored) {
        }
    }
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer uniformData = buildUniformData(stack, preparedOutlines);
            paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "UNIVERSALMOD_outline_glass_params", GpuBuffer.USAGE_UNIFORM, uniformData);
            paramsDirty = false;
            return paramsBuffer;
        }
    }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        try {
            paramsBuffer = RenderSystem.getDevice().createBuffer(
                    () -> "UNIVERSALMOD_outline_glass_params",
